...
- `docs/CLAUDE_CODE_HOMELAB_BRIEFING.txt` — fully self-contained briefing (Spanish) for a fresh Claude Code session in the homelab, no other doc needed: setup, the complete endpoint-by-endpoint mission flow with example payloads, both node-execution state machines in full, all 5 known platform faults (A–E) with full technical detail, both reference missions, the hosted-demo MCP tools, wallet/mission tables, API/MCP surface diffing, security gaps, troubleshooting table, and a verification checklist.
